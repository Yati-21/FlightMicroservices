...
class GlobalErrorHandlerTest {
	...
	void testHandleValidation() {
		...
		WebExchangeBindException exception = new WebExchangeBindException(null, result);
		StepVerifier.create(handler.handleValidation(exception)).assertNext(response -> {
	...
	void testHandleNotFound() {
		...
		StepVerifier.create(handler.handleNotFound(exception)).assertNext(response -> {
	...
	void testHandleGeneral() {
		...
		StepVerifier.create(handler.handleGeneral(exception)).assertNext(response -> {
