...
public class UserClientReactive {
    ...
    public Mono<UserDto> getUserById(String userId) {
        ...
        Mono<UserDto> call = webClientBuilder.build()
                .get()
                .uri("http://user-service/users/{id}", userId)   // ✔ CORRECT PATH
                .retrieve()
                .onStatus(HttpStatusCode::is4xxClientError,
                        r -> Mono.error(new NotFoundException("User not found")))
                .bodyToMono(UserDto.class);

        return cb.run(call, t ->
                Mono.error(new BusinessException("User service unavailable")));
    }
    ...
    public Mono<UserDto> getUserByEmail(String email) {
        ...
        Mono<UserDto> call = webClientBuilder.build()
                .get()
                .uri("http://user-service/users/email/{email}", email)   // ✔ CORRECT PATH
                .retrieve()
                .onStatus(HttpStatusCode::is4xxClientError,
