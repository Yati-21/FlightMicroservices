...
class UserClientReactiveTest {
	...
	void testGetUserById_Success() {
		...
		when(uriSpec.uri(anyString(), ArgumentMatchers.<Object[]>any())).thenReturn(headersSpec);
		when(headersSpec.retrieve()).thenReturn(responseSpec);
		when(responseSpec.onStatus(any(), any())).thenReturn(responseSpec);
		when(responseSpec.bodyToMono(UserDto.class)).thenReturn(Mono.just(user));
		when(cb.run(ArgumentMatchers.<Mono<UserDto>>any(), ArgumentMatchers.<Function<Throwable, Mono<UserDto>>>any()))
				.thenAnswer(inv -> inv.getArgument(0));

		StepVerifier.create(userClient.getUserById("U1")).expectNextMatches(u -> u.getId().equals("U1"))
				.verifyComplete();
	}
	...
	void testGetUserById_NotFound() {
		...
		when(headersSpec.retrieve()).thenReturn(responseSpec);
		when(responseSpec.onStatus(any(), any())).thenReturn(responseSpec);
		when(responseSpec.bodyToMono(UserDto.class)).thenReturn(Mono.error(new NotFoundException("User not found")));
		when(cb.run(ArgumentMatchers.<Mono<UserDto>>any(), ArgumentMatchers.<Function<Throwable, Mono<UserDto>>>any()))
				.thenAnswer(inv -> inv.getArgument(0));

		StepVerifier.create(userClient.getUserById("BAD")).expectError(NotFoundException.class).verify();
	}
	...
	void testGetUserByEmail_Success() {
		...
		StepVerifier.create(userClient.getUserByEmail("x@mail.com"))
				.expectNextMatches(u -> u.getEmail().equals("x@mail.com")).verifyComplete();
	}
}
