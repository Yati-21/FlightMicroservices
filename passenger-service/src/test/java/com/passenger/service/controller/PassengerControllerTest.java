...
public class PassengerControllerTest {
	...
	void testCreatePassenger() {
		...
		webClient.post().uri("/passengers").contentType(MediaType.APPLICATION_JSON).bodyValue(req).exchange()
				.expectStatus().isCreated().expectBody(String.class).isEqualTo("P123");
	}
	...
	void testGetByBookingId() {
		...
		Mockito.when(service.getByBookingId("B1")).thenReturn(Flux.just(p));

		webClient.get().uri("/passengers/booking/B1").exchange().expectStatus().isOk().expectBodyList(Passenger.class)
				.hasSize(1);
	}

	@Test
	void testDeleteByBooking() {
		Mockito.when(service.deleteByBookingId("B1")).thenReturn(Mono.empty());

		webClient.delete().uri("/passengers/booking/B1").exchange().expectStatus().isOk();
	}
}
